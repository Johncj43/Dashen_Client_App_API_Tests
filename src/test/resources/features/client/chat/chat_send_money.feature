Feature:Chat send_money API
  ...
  Scenario: Fail chat send money from active to no_debit account
    Given the "sender_user_active" user logs in and obtains an access token
    ...
    Then the response status code should be 201
    And the response should contain a field named "message" with the value "?????????"

  Scenario: Fail chat send money from active to dormant account
    Given the "sender_user_active" user logs in and obtains an access token
  ...
  Scenario: Fail chat send money from active to no credit account
    ...
    Then the response status code should be 400
    And the response should contain a field named "message" with the value "No credit account"
...
#    And the client sends "CHAT_SEND_MONEY_URL" request with "frozen_chat_send_money_01" to perform chat send money
  ...
  Scenario: Fail chat send money from a to frozen to credit account
    Given the "sender_user_frozen" user logs in and obtains an access token
